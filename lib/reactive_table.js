...
// For backwards compatibility - but currently broken due to template that spark can't parse
if (Handlebars) {
    Handlebars.registerHelper('reactiveTable', function (collection, settings) {
        var html = Template.reactiveTableInner(generateSettings.call({collection: collection, settings: settings}));
...
Template.reactiveTableInner.helpers({
...
Template.reactiveTable.rendered = function () {
  var frag = $(this.findAll('.editable:not(.editable-click)'));

  frag.editable('destroy').editable({
    success: function(response, newValue) {
      console.log(this, response, newValue);
    }
  }); 

  frag.on('save', function () {
    console.log('funfou!');
  });
}
